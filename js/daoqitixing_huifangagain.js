...
//	var uid = 8;
	$.ajax({
		...
		success:function(data){
			...
			$(".huifang_againbt").after('<a href="daoqitixing_huifangagain.html?omid='+data.data.omid+'"><input type="button" value="再次回访"></input></a>');
			if(history.length!=0){
				$.each(data.data.history, function(index,val) {
					$(".daoqitixing_huifangagainlist").after('<tr><td>'+val.optime+'</td><td>'+val.recom+'</td><td>'+val.quality+'</td><td>'+val.temple+'</td><td>'+val.content+'</td><td>'+val.wname+'</td></tr>');
				});
			}else{
				$(".daoqitixing_huifangagainlist").after('<tr><td>/</td><td>/</td><td>/</td><td>/</td><td>/</td><td>/</td></tr>');
			}
		}
	})
//获取url的id值
