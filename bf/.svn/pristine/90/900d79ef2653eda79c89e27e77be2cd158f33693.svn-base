		...
		$.ajax({
			...
			success:function(data){
				if (data.status==1) {
					...
					arr=sortarr(arr);
					var contentString="<tr><td>时间</td><td>新增用户</td><td>失效用户</td><td>新增体验用户</td></tr>";
		...
		$.ajax({
			...
			success:function(data){
				if (data.status==1) {
					var arr=returnCalendarDays(data.data);
					arr=sortarr(arr);
					var contentString="<tr><td>时间</td><td>SUV/MPV</td><td>精致轿车</td><td>豪华车/跑车</td></tr>";
					var newadduser=expuser=newaddexpuser="暂无数据!";
					var len=arr.length-1;
					for(var i=len;i>=0;i--){
						res=injson(arr[i],data.data);
		...
		$.ajax({
			...
			success:function(data){
				if (data.status==1) {
					var arr=returnCalendarDays(data.data);
					arr=sortarr(arr);
					var contentString="<tr><td>时间</td><td>在线支付</td><td>现金支付</td></tr>";
					var newadduser=expuser=newaddexpuser="暂无数据!";
					var len=arr.length-1;
					for(var i=len;i>=0;i--){
						res=injson(arr[i],data.data);
