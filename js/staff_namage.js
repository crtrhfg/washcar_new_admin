        ...
        $.ajax({
            ...
            success:function(data){
                ...
                $.each(data.data,function(data,val){
                    ...
                    str+=("<tr><td class='wid'>"+val.wname+"</td><td>"+val.phonenum+"</td><td>"+val.sex+"</td><td class='staff_building'>"+val.building+"</td>" +
                    "<td><div class='staff_botton'><button class='adminlist_set_change' flog="+val.wid+">修改密码</button>" +
                    "<a href='change_principal.html?wid="+val.wid+"'><button>修改</button></a><button class='adminlist_set_del' flog="+val.wid+">删除</button></div></td></tr>");
                    strhead="<tr class='admin_listcontent_mean'><td class='wid'>姓名</td><td>手机号</td><td>性别</td><td>楼盘</td><td style='min-width:230px;'>操作</td></tr>";
                });
                        ...
                        $.ajax({
                            ...
                            success:function(data){
                                if(data.status==1){
                                        ...
                                        var strhead=""; //固定头部文字
                                        $.each(data.data,function(data,val){
                                            str+=("<tr><td class='wid'>"+val.wname+"</td><td>"+val.phonenum+"</td><td>"+val.sex+"</td>"+
                                            "<td class='staff_building'>"+val.building+"</td><td><div class='staff_botton'>"+
                                            "<button class='adminlist_set_change'  flog="+val.wid+">修改密码</button>"+
                                            "<a href='change_principal.html?wid="+val.wid+"'><button>修改</button></a>"+
                                            "<button class='adminlist_set_del' flog="+val.wid+">删除</button></div></td></tr>");
                                            strhead = "<tr><td class='wid'>姓名</td><td>手机号</td><td>性别</td><td>楼盘</td><td style='min-width:230px;'>操作</td></tr>";
                                        });
                ...
                $(".adminlist_set_del").click(function(){
                    ...
                    str1='<div class="del"><p><img src="./img/warn.png"/><span>是否删除该管理</span></p>' +
                        '<button class="confirm" flog='+id+'>确定</button><button class="cancel_pass">取消</button></div>';
                    $(".admin_listcontent").append(str1);
