$(function(){
	...
	var province ="",city = "",county = "",building = "",username = "",phoneNum = "",carnum = "",park = "",rmid = "",innum = "",
		outnum = "",mstatus = "",monthchose="",totalPrice="",oncetime="",totalPriceyd="",mpricearry="",sprice="",rmidder="";
	var t = $("#text_box");
	...
	$('.rate').bind('change',function(){
		...
		if(mstatus==1){
			...
			$(".trieout,.aaasadw,.onecetime_chosebtbg,.onecetime_chosebten").css("display","none");
		}
		zzxl_calendar({												//获取多次点击的值
			box:"box1",
			lessthannowdatedisabled:false,
...
////					console.log(trimDate)
//				}
				if($(".aaasadw").css("display")=='none'){
					if(trimDate.length==innum&&this.className!="hover")return;
...
//					if(extDate.length<outnum*month&&this.className!="hover")alert('没有选择足够次数');
		...
		if($('.rate option:selected').text()=="半年卡"){
			$(".totlemoney").html(mpricearry+'元');
		}else{
			$(".totlemoney").html('金额');
		}
	});
		...
		if($("#names_").text()=="用户名格式错误"){
			...
			var config={
				...
				data:{"vid":vid,"rmid":rmidder,"rsid":rsid,"username":username,"phoneNum":phoneNum,"carnum":carnum,
					  ...
					  "totalPrice":totalPriceyd,"month":monthchose,"otherDate":otherDate,"tip":$(".beizhu").val(),"sid":sid,"omid":omid,"seid":seid,
			...
			if($(".rate option:selected").val()!="请选择服务"){
				if($(".onecetime_chosebtbg").css("display")=="none"){
					if($(".taocantime_chosebt").val()!="起始时间（单个日期）"){
						config.data.month="";
						config.data.totalPrice=mpricearry;
						$.ajax(config);
					}else{
						alert("请选择起始时间");
					}
