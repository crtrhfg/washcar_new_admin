$(function(){
	...
	$(".admin_addcontent_tjbutton").bind("click",function(){
		...
		var dancixinxi ='<div class="admin_addcontent admin_addcontent_once"><label>单次服务：</label>'+
						'<select name="cars" class="oncetc" style="width:28%" flog='+singlenum+'><option value="Provinces" style="display:none">服务项目</option></select></div>'
		...
		$('.oncetc').bind('change',function(){
			...
			var js={};
			console.log($('.timechoseryed').eq(nowflog));
		...
		zzxl_calendar({												//获取多次点击的值
			...
			onload:function(){
				...
				for(var i=0,len=extDate.length;i<len;i++){
					for(var j=0;j<oLi.length;j++){
						if(extDate[i].indexOf(yeers)>-1){
							extDate[i].split('/')[2]==$(oLi[j]).text()&&(oLi[j].className='hover');
						}
					}					
				}
				console.log(extDate)
		...
		if($("#names_").text()=="用户名格式错误"){
			...
			var config={
				...
				data:{"vid":vid,"rmid":rmidder,"rsid":rsid,"username":username,"phoneNum":phoneNum,"carnum":carnum,
					  "park":park,"carType":cartype,"startDate":startDate[0],"trimDate":trimDate,"extDate":extDate,
			...
			if($(".rate option:selected").val()=="请选择服务"){     //判断是否有数据
				...
				config.data.extDate="";
			}else if(sid!=null&&omid==null&&seid==null){
				console.log(sid)
				config.data.sid="sid";
			}else if(sid==null&&omid!=null&&seid==null){
				console.log(omid)
				config.data.omid="omid";
			}else if(sid==null&&omid==null&&seid!=null){
				console.log(seid)
				config.data.seid="seid";
			}
			$.ajax(config)	
			rsid.length=0;
